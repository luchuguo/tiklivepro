...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (cachedData) {
      console.log(`返回缓存的任务申请列表: ${taskId}`);
      return res.status(200).json(cachedData);
    }

    console.log(`从 Supabase 获取任务申请列表: ${taskId}`);
    console.log('环境变量状态:', { 
    ...
    if (error) {
      console.error(`Supabase 查询任务申请列表失败: ${taskId}`, error);
      return res.status(500).json({ error: error.message });
    }

    console.log(`成功获取任务申请列表: ${taskId}, 共 ${data?.length || 0} 条申请`);
