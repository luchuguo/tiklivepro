...
DECLARE
  -- 使用固定的UUID作为模拟用户ID
  influencer1_id uuid := 'b1111111-1111-4111-8111-111111111111'::uuid;
  influencer2_id uuid := 'b2222222-2222-4222-8222-222222222222'::uuid;
  influencer3_id uuid := 'b3333333-3333-4333-8333-333333333333'::uuid;
  influencer4_id uuid := 'b4444444-4444-4444-8444-444444444444'::uuid;
  influencer5_id uuid := 'b5555555-5555-4555-8555-555555555555'::uuid;
  
  company1_id uuid := 'c1111111-1111-4111-8111-111111111111'::uuid;
  company2_id uuid := 'c2222222-2222-4222-8222-222222222222'::uuid;
  company3_id uuid := 'c3333333-3333-4333-8333-333333333333'::uuid;
...
BEGIN
  ...
  ALTER TABLE tasks DROP CONSTRAINT IF EXISTS tasks_selected_influencer_id_fkey;
  ALTER TABLE task_applications DROP CONSTRAINT IF EXISTS task_applications_task_id_fkey;
  ALTER TABLE task_applications DROP CONSTRAINT IF EXISTS task_applications_influencer_id_fkey;
  ...
  ALTER TABLE live_sessions DROP CONSTRAINT IF EXISTS live_sessions_task_id_fkey;
  ALTER TABLE live_sessions DROP CONSTRAINT IF EXISTS live_sessions_influencer_id_fkey;
  ...
  -- 获取达人ID
  SELECT id INTO inf1_id FROM influencers WHERE user_id = influencer1_id;
  SELECT id INTO inf2_id FROM influencers WHERE user_id = influencer2_id;
  SELECT id INTO inf3_id FROM influencers WHERE user_id = influencer3_id;
  SELECT id INTO inf4_id FROM influencers WHERE user_id = influencer4_id;
  SELECT id INTO inf5_id FROM influencers WHERE user_id = influencer5_id;
  ...
  -- 获取企业ID
  SELECT id INTO comp1_id FROM companies WHERE user_id = company1_id;
  SELECT id INTO comp2_id FROM companies WHERE user_id = company2_id;
  SELECT id INTO comp3_id FROM companies WHERE user_id = company3_id;
  ...
  INSERT INTO tasks (
    ...
    max_applicants, current_applicants, views_count
  ) VALUES
    (
      comp1_id,
      '新品口红直播推广', 
  ...
  -- 获取任务ID
  SELECT id INTO task1_id FROM tasks WHERE title = '新品口红直播推广';
  SELECT id INTO task2_id FROM tasks WHERE title = '护肤套装春季促销';
  SELECT id INTO task3_id FROM tasks WHERE title = '夏季新款连衣裙直播';
  SELECT id INTO task4_id FROM tasks WHERE title = '智能手表新品发布';
  SELECT id INTO task5_id FROM tasks WHERE title = '运动鞋品牌合作';
  ...
  -- 创建评价记录
  INSERT INTO reviews (
    task_id, reviewer_id, reviewee_id, reviewer_type, rating, comment
  ) VALUES
    (
  ...
  -- 重新创建外键约束（注意：这些约束在实际环境中可能不需要，因为我们使用的是模拟数据）
  -- 在生产环境中，这些外键约束应该指向 auth.users 表
  -- 这里我们跳过重新创建外键约束，因为我们使用的是测试数据
